...
using Weixin.CustomMessageHandler.ApiStore;

namespace Weixin.CustomMessageHandler.MessageHandlers.CustomMessageHandler
{
    /// <summary>
    /// 自定义MessageHandler
    ...
    public partial class CustomMessageHandler : MessageHandler<CustomMessageContext>
    {
        ...
        public override IResponseMessageBase OnTextRequest(RequestMessageText requestMessage)
        {
            ...
            else if (requestMessage.Content.Contains("天气"))
            {
                try
                {
                    string[] city = requestMessage.Content.Split('，');
                    var result = string.Empty;
                    for (int i = 0; i < city.Length; i++)
                    {
                        if (city[i]!="天气")
                            result= WeatherApi.GetCityWeather(city[i]);
                    }
